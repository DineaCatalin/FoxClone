...
public class CharacterController : MonoBehaviour
{
	...
	[SerializeField] private float m_FallTreshhold = -4f;
	...
	public UnityEvent OnStartFallingEvent;
	...
	private void FixedUpdate()
	{
        ...
        if (m_Rigidbody2D.velocity.y < m_FallTreshhold && !m_IsFalling && !m_Grounded)
        {
            //Debug.Log($"CDI We are falling m_Rigidbody2D.velocity.y {m_Rigidbody2D.velocity.y} m_startFalling {m_IsFalling}");

            m_IsFalling = true;

            if (!wasFalling)
            {
                OnStartFallingEvent?.Invoke();
            }
        }
    }
